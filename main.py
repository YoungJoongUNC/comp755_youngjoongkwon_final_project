# -*- coding: utf-8 -*-
"""
Created on Sun Nov 18 15:36:04 2018
...
class FabricTrainingDataset(Dataset):
    def __init__(self):
        """
        Args:
        ...
        self.image_folder_path = "D:/MIT Fabric Dataset Processed/"
        # Transforms
        self.to_tensor = transforms.ToTensor()
        self.to_square = transforms.Resize((imagesize,imagesize))
        # Read the (videoindex,label) file
        gt = []
        f = open("D:/MIT Fabric Dataset Index/training_dataset.txt", 'r')
        lines = f.readlines()
        for line in lines:
...
class FabricValidationDataset(Dataset):
    def __init__(self):
        """
        Args:
        ...
        # Read the (videoindex,label) file
        gt = []
        f = open("D:/MIT Fabric Dataset Index/validation_dataset.txt", 'r')
        lines = f.readlines()
        for line in lines:
            index, label = line.split(",")
            label=label[0:-1]
...
class FabricTestingDataset(Dataset):
    def __init__(self):
        ...
        # Read the (videoindex,label) file
        gt = []
        f = open("D:/MIT Fabric Dataset Index/testing_dataset.txt", 'r')
        lines = f.readlines()
        for line in lines:
            index, label = line.split(",")
            label=label[0:-1]
...
training_dataset = FabricTrainingDataset()
validation_dataset = FabricValidationDataset()
testing_dataset = FabricTestingDataset()

training_loader = torch.utils.data.DataLoader(training_dataset, batch_size=training_batch_size, shuffle=True, num_workers=0)
validation_loader = torch.utils.data.DataLoader(validation_dataset, batch_size=validating_testing_batch_size, shuffle=True, num_workers=0)
    ...
    for batch_idx, (data, target) in enumerate(training_loader):
        ...
        if batch_idx % args.log_interval == 0:
            print('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(
                ...
                100. * batch_idx / len(training_loader), loss.data[0]))
            f = open("D:/MIT Fabric Dataset Training Loss Accuracy/"+test_index+str(epoch)+".txt",'a')
            f.write('{:.6f}'.format(loss.data[0])+"\n")
            f.close()
...
def validate(epoch_idx):
    ...
    print(
        '\nTest set: Average loss: {:.4f}, Accuracy: {}/{} ({:.0f}%)\n'.format(
            validate_loss, correct, len(validation_loader.dataset),
            100. * correct / len(validation_loader.dataset)))
    f = open("D:/MIT Fabric Dataset Validation Loss Accuracy/"+test_index+str(epoch_idx)+".txt",'a')
    f.write(str(validate_loss)+","+str(100. * correct / len(validation_loader.dataset))+"\n")
    f.close()
...
def test(epoch_idx):
    ...
    f = open("D:/MIT Fabric Dataset Testing Loss Accuracy/"+test_index+str(epoch_idx)+".txt",'a')
    f.write(str(test_loss)+","+str(100. * correct / len(testing_loader.dataset))+"\n")
    f.close()

ROOTPATH = "D:/MIT Fabric Models/"
for epoch in range(1, args.epochs + 1):
    torch.save(model.state_dict(), ROOTPATH+"test0_"+"before_"+str(epoch)+".pt")
    torch.save(model.state_dict(), ROOTPATH+"test0_"+"before_"+str(epoch)+".pth")
    train(epoch)
    validate(epoch)
    #test(epoch)
    torch.save(model.state_dict(), ROOTPATH+"test0_"+"after_"+str(epoch)+".pt")
    torch.save(model.state_dict(), ROOTPATH+"test0_"+"after_"+str(epoch)+".pth")
